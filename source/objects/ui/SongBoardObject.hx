...
class SongBoardObject extends FlxTypedGroup<FlxSprite> {
    ...
    private var _canTween:Bool = false;

    private var _id:String;  // The ID of the song
    ...
    private var _bg:ClickableSprite;
    ...
    public function new(y:Float, id:String, name:String, bpm:Int, speed:Float, difficulty:Int, bannerColor:Array<Int>) {
        ...
        _bg.alpha = 0.65;
        _bg.updateHitbox();
        _bg.setPosition((FlxG.width - _bg.width) - 40, y);
        _bg.onClick = () -> {
            if (isFocusedOn) {
                GeneralUtil.fadeIntoState(new PlayState(id), Constants.TRANSITION_DURATION, false);
            }
        }
        insert(0, _bg);  // Make sure the banner is in the back!
    }
    ...
    override function update(elapsed:Float) {
        ...
        if (isFocusedOn) {
            if (Controls.getBinds().UI_SELECT_JUST_PRESSED) {
                GeneralUtil.fadeIntoState(new PlayState(_id), Constants.TRANSITION_DURATION, false);
            }
        }
    }
    ...
    public function setTextPositions():Void {
        _songNameText.setPosition(_bg.x + 2, _bg.y + 2);
        _songDifficultyText.setPosition(_bg.x + 2, (_songNameText.y + _songNameText.height) - 6);
        _songBPMText.setPosition(_bg.x, (_songDifficultyText.y + _songDifficultyText.height) - 6);
